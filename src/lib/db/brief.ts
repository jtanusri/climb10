...
export async function getBrief(): Promise<Brief | null> {
  const db = getDb();
  const result = await db.execute('SELECT * FROM brief WHERE id = 1');
  return (result.rows[0] as unknown as Brief) ?? null;
}

export async function upsertBrief(data: Partial<Brief>): Promise<Brief> {
  const db = getDb();
  const existing = await getBrief();
  ...
  if (existing) {
    ...
    await db.execute({
      sql: `INSERT INTO brief (id, geography, radius_miles, budget_display_note, headcount_display_note,
